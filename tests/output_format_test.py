...
def test_channels_list_empty(user):
...
def test_channels_listall_empty(user):
...
def test_channel_join(user, channel):
	out = channel_join_v1(user, channel)
	assert isinstance(out, dict)
	assert len(out.keys()) == 0

def test_channel_invite(user, channel):
	user2 = auth_register_v1("user2@mail.com", "password", "first", "last")['auth_user_id']
	out = channel_invite_v1(user, channel, user2)
	assert isinstance(out, dict)
	assert len(out.keys()) == 0
