...
def dm(user1, user2):
	dm = dm_create_v1_request(user1, [user2]).json()['dm_id']
	message_send_v1_request(user1, dm, "the world is flat")
	message_send_v1_request(user2, dm, "you're an idiot")
...
def test_one_result(user1, channel1):
	...
	assert {m['messsage'] for m in messages} == {"hello world"}

def test_multiple_results(user1, channel1):
	messages = search_v1_request(user1, "defg").json()['messages']
	assert {m['messsage'] for m in messages} == {"abcdefg", "defghij"}
...
def test_dm(user1, dm):
	messages = search_v1_request(user1, "world").json()['messages']
	assert {m['messsage'] for m in messages} == {"the world is flat"}

def test_dm_and_channel(user1, dm, channel1):
	messages = search_v1_request(user1, "world").json()['messages']
	assert {m['messsage'] for m in messages} == {"the world is flat", "hello world"}
