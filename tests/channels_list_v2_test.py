...
def test_channels_list_basic():
    user_1 = auth_register_v2_request("user1@gmail.com", "password1", "firstname", "lastname").json()['auth_user_id']
    channels_create_v2_request(user_1, "channel", True)

    assert channels_list_v2_request(user_1) == {
        'channels': [
            {
                'channel_id': 0,
                'name': 'channel',
...
def test_channels_list_multiple():
    user_1 = auth_register_v2_request("user1@gmail.com", "password1", "firstname", "lastname").json()['auth_user_id']
    user_2 = auth_register_v2_request("user2@gmail.com", "password2", "firstname", "lastname").json()['auth_user_id']
    ...
    assert channels_list_v2_request(user_2) == {
        'channels': [
            {
                'channel_id': 2,
                'name': 'channel3',
...
def test_channels_list_private():
    user_1 = auth_register_v2_request("user1@mail.com", "password", "first", "last").json()['auth_user_id']
    user_2 = auth_register_v2_request("user2@mail.com", "password", "first", "last").json()['auth_user_id']
    ...
    assert channels_list_v2_request(user_2) == {'channels': []}

    channel_invite_v2_request(user_1, channel_1, user_2)
...
def test_invalid_auth_id():
	with pytest.raises(AccessError):
		assert channels_list_v2_request(1234)
