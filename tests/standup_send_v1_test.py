...
@pytest.fixture
def user():
    return auth_register_v2_request("u@mail.com", "psword", "blake", "morris").json()['token']
...
def test_status(user, channel):
	...
	assert standup_send_v1_request(user, channel, "my catfish ate a kmart toaster").status_code == 200
...
def test_invalid_length(user, channel):
	...
	assert standup_send_v1_request(user, channel, "a" * 1000).status_code == 200
	assert standup_send_v1_request(user, channel, "a" * 1001).status_code == 400
...
def test_invalid_token(user, channel):
	...
	assert standup_send_v1_request("qwerty", channel, "hello").status_code == 403

	assert standup_send_v1_request("~" + user[1:], channel, "hello").status_code == 403
