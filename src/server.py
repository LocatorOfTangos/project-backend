...
from src.user import user_profile_v1, user_profile_sethandle_v1, user_profile_setemail_v1, user_profile_setname_v1, user_stats_v1
from src.message import message_edit_v1, message_react_v1, message_send_v1, message_remove_v1, message_senddm_v1, message_unreact_v1, message_pin_v1, message_unpin_v1, message_sendlater_v1, message_sendlaterdm_v1
...
def message_sendlater():
    data = request.get_json()
    resp = message_sendlater_v1(data['token'], data['channel_id'], data['message'], data['time_sent'])
    return dumps(resp)

@APP.route('/message/sendlaterdm/v1', methods=['POST'])
def message_sendlaterdm():
    data = request.get_json()
    resp = message_sendlaterdm_v1(data['token'], data['dm_id'], data['message'], data['time_sent'])
    return dumps(resp)
